...
export default {
  ...
  methods: {
    async postEntryModal() {
      ...
      try {
        const response = await axios.post("http://localhost:3000/api/addfood", entry, {
          headers: {
    ...
    async refreshData() {
      try {
        const response = await axios.get("http://localhost:3000/api/foodlist");
    ...
    async deleteEntry(date, name) {
      ...
      try {
        const response = await axios.post(
          "http://localhost:3000/api/deletefood",
          entryToDelete,
          {
            headers: {
              "Content-Type": "application/json",
